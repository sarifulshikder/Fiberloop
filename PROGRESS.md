...
**Network Device form — dedicated port fields (Post-Phase 19)**: The Add/Edit Network Device form was reorganized so the **Network Connectivity** section holds all connection fields: IP Address, Hostname, Username, Password, **API Port** (`port`), **SSH Port** (`ssh_port`), **Telnet Port** (`telnet_port`); the **SNMP Port** (`snmp_port`) lives in the **Management Protocol** section with the protocol select + SNMP community/version. **All four port fields are blank by default and optional** — the admin enters a number only when that port is actually needed; every transport falls back to sane defaults when unset (`port`→8728 via `api_port` accessor, `ssh_port ?: port ?: 22` in `CliTransport`, `telnet_port ?? config('olt.telnet_port', 23)` in VSOL/BDCOM drivers, `snmp_port ?? 161` in SNMP drivers). Migrations: `2026_08_14_120000_add_ssh_and_telnet_ports_to_network_devices_table` (adds `ssh_port` + `telnet_port`, backfills `telnet_port` from legacy `configuration.telnet_port` JSON: Vsol→223, Bdcom→225, Core Router→23), `2026_08_14_130000_set_default_ports_on_network_devices_table` (DB defaults port 8728/ssh 22/telnet 23), and `2026_08_14_140000_make_network_device_ports_nullable` (**final state: all four port columns nullable with no default** — verified via `information_schema`: nullable=YES default=NULL). Model fillable + integer casts; factory uses null ports; drivers read `telnet_port` column instead of `configuration.telnet_port`. **Verification:** `tests/Feature/NetworkDeviceFormTest.php` → **5 passed (44 assertions)** incl. new "creates a device with all port fields left blank" (Livewire create → record persists with null ports). `tests/Unit/Network/` → **63 passed (302 assertions)**. Real-dev-DB HTTP smoke (temp Pest test against `fiberloop`, deleted): create page 200, SNMP Port label renders AFTER the Management Protocol section, and no `value="8728"/"22"/"23"/"161"` defaults in the port inputs. Browser check: Network → Network Devices → Add → all four ports empty; SNMP Port under Management Protocol.
...
- Phase 19 / BDCOM P3310C telnet wiring verified against live hardware (2026-08-14): OLT id 6 (`Bdcom P3310C`, device 9, `103.112.150.52`, telnet **225**, creds `Sariful`) was fully wired and verified live — **"Sync Now" and "Poll Ports" both work end-to-end in the browser** (`/admin/olts` id 6, `/admin/onus`, `/admin/olt-ports`).
  - **`TelnetTransport` is now prompt/login-configurable.** Constructor gained `promptPattern` (default VSOL `epon-olt...`) and `loginPromptPattern` (default `/login:\s*$/i`); BDCOM passes `Switch(?:\(config[^)]*\))?[#>]` and `/username:\s*$/i` (its banner is "User Access Verification" → `Username:`). Also sends `terminal width 0` after `terminal length 0` — BDCOM wraps table rows at 80 columns, which corrupted row parsing; unsupported vendors just echo an error and re-prompt (safe).
  - **`config/olt.php` bdcom commands corrected from docs-best-effort to probe-verified:** `onu_info`=`show epon onu-information interface epon0/%s`, `onu_optical`=`show epon onu-ctc-optical-transceiver-diagnosis interface epon0/%s`, `onu_descriptions`=`show running-config`, `pon_info`=`show interface epon0/%s`, `gigabit_info`=`show interface GigaEthernet0/%s`. `%s` = bare PON port number (`1`..`4`), NOT `0/1` form.
  - **`OltCliOutputParser` additions:** dotted-MAC detection (`6c68.a46f.6b18` Cisco format → `6C:68:A4:6F:6B:18`; old `[0-9A-Fa-f]{2}[:-]` regex silently missed it because `[:-]` is a range excluding `.`); BDCOM section-header filtering (`Interface EPON0/1 has registered 18 ONUs:` would otherwise parse as a phantom ONU with serial `INTERFACE`); `auto-configured`/`auto-configuring` state words (`auto-configured` → online+registered); `parseBdcomDescriptionsTable()` (per-ONU `interface EPON0/1:1` + `description <name>` blocks); `parseBdcomInterfaceInfo()` (Cisco-style `X is up, line protocol is up` + `Description:`/`Hardware is`/`MTU`/`Auto-Speed`).
  - **`BdcomCliDriver`** now implements `SupportsCliPortPoll`, builds the telnet transport (port from `network_devices.configuration['telnet_port']`), maps `pon_ports` → bare port numbers, defaults 6 GE ports (P3310C has no 0/7-0/8), parses descriptions via the BDCOM parser, and classifies uplinks from `olts.configuration['uplink_ports']` (set to `0/1,0/4,0/5,0/6` from the live `dot1q-tunnel-uplink` running-config) OR a `link`/`uplink` description.
  - **Live verification (raw):** `discoverOnus()` → **23 ONUs** (18 on `0/1`, 5 on `0/3`), every row with normalized MAC + customer_name from running-config + RX/TX dBm; 0 bad rows. `OltSyncService::sync()` → `{"discovered":23,"created":9,"updated":14,"signal_ok":23,"signal_fail":0,"reachable":true}`, `used_pon_ports`=2. `pollPorts()` → `{"polled":10,"created":10,"updated":0,"reachable":true}`; re-poll idempotent `{"created":0,"updated":10}`. Ports: `EPON0/1-4` (if_index 1-4, oper from `is up/down`), `GE0/1-6` (if_index 101-106, uplink: GE0/1,0/4,0/5,0/6; access: GE0/3; other: GE0/2). **SNMP on that IP is the VSOL** (sysDescr "V1600D", enterprise 37950) — the BDCOM has no SNMP of its own, hence CLI port polling.
  - **Data-model fix (user-approved):** OLT 5 (`Vsol`) and OLT 6 (`Bdcom`) both point at `103.112.150.52`, so the same physical ONUs are visible from both records, and `onus.serial_number`/`mac_address` were **globally unique** → BDCOM sync collided with OLT 5's rows. New migration `2026_08_14_123742_make_onu_serial_and_mac_unique_per_olt` drops the global uniques and adds `onus_olt_id_serial_number_unique` + `onus_olt_id_mac_address_unique` (intra-OLT duplicates verified = 0 before creating indexes; working `down()` restores globals).
  - Tests: `tests/Unit/Network/` now **63 passed (302 assertions)**; new `BdcomCliDriverTest` (5 tests) mirrors the VSOL fixture pattern, `OltCliOutputParserTest` +5 BDCOM fixtures, factory test +1 (bdcom+ssh → `BdcomCliDriver`). Full suite: **211 passed / 28 failed (all pre-existing) / 1 risky / 2 skipped** — no new failures. Pint clean. Octane reloaded before browser verification.
