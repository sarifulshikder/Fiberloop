    ...
    protected function getStats(): array
    {
        return [
            Stat::make('New Leads', Lead::query()->where('status', LeadStatus::NEW)->count())
                ...
                ->color('info'),
            Stat::make('Contacted', Lead::query()->where('status', LeadStatus::CONTACTED)->count())
                ->description('Leads contacted')
                ->descriptionIcon('heroicon-o-phone')
                ->color('primary'),
            Stat::make('Site Survey', Lead::query()->where('status', LeadStatus::SITE_SURVEY)->count())
                ->description('Leads in site survey')
                ->descriptionIcon('heroicon-o-map-pin')
                ->color('warning'),
            Stat::make('Converted', Lead::query()->where('status', LeadStatus::CONVERTED)->count())
